...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: links } = ids.length ? await auth.client.from("product_media").select("product_id, role, media:media_id(storage_key, mime_type, alt_text)").in("product_id", ids).order("display_order") : { data: [] };
