...
export default function ProductForm({ mode = "create", productId, initialProduct }: { mode?: "create" | "edit"; productId?: string; initialProduct?: Record<string, unknown> }) {
  ...
  return <form onSubmit={submit} className="mt-8 grid gap-6 rounded-xs border border-line bg-white p-6">
    ...
    <section className="grid gap-4 rounded-xs border border-line bg-cream p-4 sm:grid-cols-2">
      ...
      {/*
        Jewellery Category is the primary merchandising class and decides which
        of /gold-jewellery, /silver-jewellery, /diamond-jewellery the product
        appears on. It is NOT the jewellery type above (Rings, Earrings), and
        NOT metal purity: a diamond ring mounted in 18K gold is "Diamond".
        Required, with no default -- the server rejects a missing value rather
        than guessing one.
      */}
      <label className="text-sm font-medium">Jewellery category<Required /><select className={CONTROL_ON_CREAM} value={form.jewelleryCategory} onChange={(event) => setField("jewelleryCategory", event.target.value)} required><option value="">Select</option><option value="gold">Gold</option><option value="silver">Silver</option><option value="diamond">Diamond</option><option value="platinum">Platinum</option></select><span className="mt-1 block text-xs font-normal text-muted">Which collection page this appears on. A diamond piece set in 18K gold is Diamond, not Gold.</span></label>
