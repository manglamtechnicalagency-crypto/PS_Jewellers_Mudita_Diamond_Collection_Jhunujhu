  ...
  return <>
    <header className="sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-content items-center justify-between gap-4 px-4 sm:px-5 lg:h-20 lg:gap-6 lg:px-10">
        <a className="font-serif text-lg font-semibold tracking-tight text-ink sm:text-xl" href="/" aria-label="PS Jewellers home">PS <span className="text-gold-500">Jewellers</span></a>
        ...
        <div className="hidden items-center gap-5 lg:flex">
          ...
          <a href="/account" aria-label="Account" className="text-sm font-medium text-ink-soft hover:text-gold-600">Account</a>
        </div>
    ...
    <div id="mobile-navigation" className={`fixed inset-x-0 bottom-0 top-16 z-30 overscroll-contain bg-paper transition-opacity duration-200 lg:hidden ${open ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"}`} aria-hidden={!open} inert={!open}>
      <div className="flex h-full flex-col gap-6 overflow-y-auto px-5 py-6">
        ...
        <div className="mt-auto border-t border-line pt-5 text-sm font-medium text-ink-soft"><a href="/account" onClick={closeMenu} className="flex min-h-11 items-center justify-center rounded-xs border border-line">Account</a></div>
      </div>
    </div>
  </>;
}
