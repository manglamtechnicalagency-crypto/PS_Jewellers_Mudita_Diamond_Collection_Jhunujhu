...
export default function Header({ appState }: HeaderProps) {
  ...
  return <>
    <header className="sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur">
      <div className="mx-auto flex h-20 max-w-content items-center justify-between gap-4 px-4 sm:px-5 lg:h-24 lg:gap-6 lg:px-10">
        <a className="flex shrink-0 items-center" href="/" aria-label="PS Jewellers — Mudita Diamonds, home"><BrandLogo className="h-[70px] w-auto lg:h-20" priority /></a>
    ...
    <div id="mobile-navigation" className={`fixed inset-x-0 bottom-0 top-16 z-30 overscroll-contain bg-paper transition-opacity duration-200 lg:hidden ${open ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"}`} aria-hidden={!open} inert={!open}>
        ...
        <div className="flex items-center justify-between border-b border-line pb-4"><BrandLogo className="h-20 w-auto" /><small className="text-muted">Jhunjhunu, Rajasthan</small></div>
